...
export const PromptInput: React.FC<PromptInputProps> = ({
  ...
  const availableDropdownMethods = useMemo(() => {
    ...
    const builtinInteractionMethods = new Set(['aiAct', 'runYaml']);
    ...
    // Add filtered apiMetadata methods
      ...
      if (
        ...
        if (
          ...
          builtinInteractionMethods.has(method)
        ) {
          finalMethods.add(method);
        }
      }
  ...
  const getDefaultParams = useCallback((): FormParams => {
    ...
    return {};
  }, [disableDefaultParams, selectedType, needsStructuredParams, actionSpace]);
  ...
  useEffect(() => {
    ...
    if (lastHistory) {
      ...
      form.setFieldsValue({
        prompt: '',
        params: disableDefaultParams ? {} : getDefaultParams(),
      });
      setPromptValue('');
      lastHistoryRef.current = null;
    }
  }, [
    disableDefaultParams,
    selectedType,
  ...
  const handleRunWithHistory = useCallback(() => {
    ...
    if (clearPromptAfterRun) {
      ...
      if (needsStructuredParams) {
        form.setFieldValue(
          'params',
          disableDefaultParams ? {} : getDefaultParams(),
        );
      } else {
        ...
        if (schemaKeys.length === 1) {
          ...
          const placeholderText = (() => {
            ...
            if (isLocateFieldFlag) {
              ...
              if (key === 'keyName') return '请输入按键名或要输入的文本';
              if (key === 'value') return '请输入要填写的内容';
              return `请输入 ${key}`;
            }
          })();
  ...
  const renderActionButton = useCallback(() => {
    ...
    if (dryMode) {
      return selectedType === 'aiAct' ? (
        <Tooltip title="开始执行到需要交互动作前停止，你可以查看规划与定位过程。">
          {runButton('演练')}
        </Tooltip>
      ) : (
        runButton('运行')
    ...
    <div className="prompt-input-wrapper">
      ...
      >
        {needsAnyInput ? (
          ...
          >
            点击“运行”以执行 {actionNameForType(selectedType)}
          </div>
        )}
