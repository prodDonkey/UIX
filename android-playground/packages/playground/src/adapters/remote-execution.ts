...
export class RemoteExecutionAdapter extends BasePlaygroundAdapter {
  ...
  async getTaskResult(requestId: string): Promise<{
    requestId?: string;
    status?: string;
    result?: unknown;
    ...
    if (!requestId?.trim()) {
      console.warn('Invalid requestId provided for task result');
      return {};
    }

    try {
      const response = await fetch(
        `${this.serverUrl}/task-result/${encodeURIComponent(requestId)}`,
      );
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          return {
            ...
            error: body?.error || 'Task result not found',
          };
        }
        console.warn(`Task result request failed: ${response.statusText}`);
        return {};
      }
